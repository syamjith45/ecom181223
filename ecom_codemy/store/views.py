...
# Assuming your Customer model has a foreign key to the User model named 'user'
    order = get_object_or_404(Order, customer=request.user.customer, status=False)
...
def create_customer(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('product')  # Redirect to a success page or any other view
    else:
        form = CustomerForm()
